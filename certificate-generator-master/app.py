...
FROM_EMAIL = 'adcmusicdepartment@gmail.com'  # SET THIS
FROM_EMAIL_PASSWORD = 'inupakhi'  # SET THIS
...
BLANK_LIGHT = (FROOT + 'blank-light.jpg')
BLANK_DARK = (FROOT + 'blank-dark.jpg')
BLANK_CUSTOM = resource_path(FROOT + 'blank-dark.jpg')

# Certificate fonts
FONTL = ('PlayfairDisplay-VariableFont_wght.ttf')
FONTM = ('PlayfairDisplay-VariableFont_wght.ttf')

# This is the logo in the email
PSLOGO = (FROOT + 'dars.png')
# GUI app icon
myicon = (FROOT + '@myicon.xbm')
...
def print_certificate(name, college, event, logo, bgmode):
    ...
    img = Image.open('Sample Certificate DARS.jpg')
    draw = ImageDraw.Draw(img)
    fontl = ImageFont.truetype(FONTL, size=72)
    fontm = ImageFont.truetype(FONTM, size=27)
    color = '#0000' #black color
    ...
    for i in range(0, 5):
        ...
        if i == 0:
            ...
            draw.text((x, y), text[0], fill=color, font=fontl)
            draw.text((x, y), ' %s ' % name, fill=color, font=fontm)
            draw.text((x, y), text[1], fill=color, font=fontl)
            ...
            draw.text((x, y), ' %s ' % college, fill=color, font=fontm)
            draw.text((x, y), text[2], fill=color, font=fontl)
            ...
            draw.text((x, y), ' %s ' % event, fill=color, font=fontm)
            draw.text((x, y), text[3], fill=color, font=fontl)
            ...
            draw.text((x, y), out, fill=color, font=fontl)
...
def generate(mode):
    ...
    for i in range(0, sheet.nrows):
        ...
        if college in ['REC', 'RECB']: college = 'Rajkiya Engineering College'
        elif college in ['AKTU']: college = 'A.P.J.Abdul Kalam Technical University'
...
def button_trigger(mode):
    ...
    try:
        ...
        ERROR_LOG += '>>> ' + str(err) + '\n\n'
        tkinter.messagebox.showwarning('Unexpected Error Occured', ERROR_LOG + 'Please report the error(s) to shivam.sagar.shivam@gmail.com')
...
root = Tk()
root.title('E-Certificate Generator for An Event Participants')
root.minsize(width=465, height=310)
root.maxsize(width=465, height=310)
root.iconbitmap(default=myicon)
...
info = 'e-Certificate Generator by Devesh, Akhil, Shivam, Rahul'
